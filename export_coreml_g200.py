...
cycleGAN.netG_A = cycleGAN.netG_A.to(device).half()
model_netG_A = cycleGAN.netG_A
print("Model loaded successfully.")

# ───────────────────────────────────────────────────────────────
# Step 4: Trace the model
dummy_input = torch.randn(1, 3, inference_size, inference_size, dtype=torch.float16).to(device)
model_netG_A.eval()
traced_model = torch.jit.trace(cycleGAN.netG_A, dummy_input)
